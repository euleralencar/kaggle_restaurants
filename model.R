...
setwd("C:\\Users\\t318824.TRIBUNAL\\Desktop\\Renato\\Programas R\\Kaggle\\Restaurant Revenue Prediction")

library(MASS)
library(cvTools)
library(DAAG)

train <- read.table("train.csv", head = T, sep = ",", encoding="UTF-8")
head(train)
levels(train$City)
attach(train)

test <- read.table("test.csv", head = T, sep = ",", encoding="UTF-8")
...
plot(tmp, ylab="Raiz do módulo do escore", xlab="Valor ajustado", pch=20) ;  grid()
...
plot(hatvalues(mod.wls1), pch=20, xaxt="n", xlab="", ylab="valores-chapéu")  ;  grid()
abline(h=c(.5, 2 * p / n), lty=2)  

plot(cooks.distance(mod.wls1), pch=20, xaxt="n", xlab="", ylab="Distância de Cook´s")  ;  grid()
...
plot(hatvalues(mod.wls2), pch=20, xaxt="n", xlab="", ylab="valores-chapéu")  ;  grid()
abline(h=c(.5, 2 * p / n), lty=2)  

plot(cooks.distance(mod.wls2), pch=20, xaxt="n", xlab="", ylab="Distância de Cook´s")  ;  grid()
...
plot(hatvalues(mod.wls3), pch=20, xaxt="n", xlab="", ylab="valores-chapéu")  ;  grid()
abline(h=c(.5, 2 * p / n), lty=2)  

plot(cooks.distance(mod.wls3), pch=20, xaxt="n", xlab="", ylab="Distância de Cook´s")  ;  grid()
...
#modelos que consideram City não são adequados, pois muitas cidades tem apenas uma unidade na
#amostra de treino
...
#alguns são negativos!
head(Prediction,1000)

Id <- 0:99999
submission <- data.frame(Id, Prediction)

write.table(submission, "Submision.csv", row.names = F, sep = ",")
...
# ver http://statistics.ats.ucla.edu/stat/r/dae/rreg.htm para exemplos de regressão robusta
